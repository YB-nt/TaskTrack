...
| 2026-08-01 | Today 목록에서 섹션(서브태스크) 내 뒤쪽 in-progress 테이블 행이 누락되던 버그 수정 — `ScheduleEngine`이 섹션당 대표 leaf를 "첫 번째 미완료" 대신 "in-progress 우선"으로 선택하도록 변경, 재현 회귀 테스트 추가(74/74 통과, 수정 전 코드로는 해당 테스트만 실패하는 것 확인). 부가로 프로젝트 `.claude/settings.json`에 `worktree.baseRef: "head"` 추가 (이 저장소는 기능 브랜치가 origin/main이 아니라 develop에서 갈라지므로) | Sources/TaskflowCore/Scheduling/ScheduleEngine.swift, Sources/TaskflowVerify/main.swift, .claude/settings.json | 사용자가 "Today에 진행중인 내용이 안 보인다" 보고 — tasks_v2.md류 3단계 문서에서 한 서브태스크에 여러 테이블 행(문제 X-Y)이 있을 때 문서 순서상 앞선 행이 아직 pending이면, 뒤에 있는 실제 in-progress 행이 Today 대표 항목 선정에서 아예 제외되는 구조적 결함이었음 |
